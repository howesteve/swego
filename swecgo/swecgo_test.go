...
func Test_wrapper_NodAps(t *testing.T) {
	...
	cases := []struct {
		...
		{wrapperPtr.NodAps, swego.NodbitMean, result{
		...
		{wrapperPtr.NodApsUT, swego.NodbitMean, result{
		...
		{wrapperPtr.NodAps, swego.NodbitMean | swego.NodbitFoPoint, result{
			[]float64{125.067162, .0, .002461, .0, .0, .0},
			[]float64{305.067162, .0, .002671, .0, .0, .0},
			[]float64{83.408587, -3.425232, .002428, .0, .0, .0},
			[]float64{263.408587, 3.425232, .000282, .0, .0, .0}, // different
		}},
		{wrapperPtr.NodApsUT, swego.NodbitMean | swego.NodbitFoPoint, result{
			[]float64{125.067123, .0, .002461, .0, .0, .0},
			[]float64{305.067123, .0, .002671, .0, .0, .0},
			[]float64{83.408669, -3.425224, .002428, .0, .0, .0},
...
func Test_wrapper_GetAyanamsa(t *testing.T) {
	...
	cases := []struct {
		...
		{wrapperPtr.GetAyanamsa, 24.740393},
		{wrapperPtr.GetAyanamsaUT, 24.740393},
	}
...
func Test_wrapper_GetAyanamsaEx(t *testing.T) {
	...
	cases := []struct {
		...
		{wrapperPtr.GetAyanamsaEx, 24.740393},
		{wrapperPtr.GetAyanamsaExUT, 24.740393},
	}
...
var housesTests = []struct {
	in   housesTestInput
	want housesTestResult
}{
	{
		housesTestInput{52.083333, swego.Placidus, nil},
		housesTestResult{
			[]float64{0,
	...
	{
		housesTestInput{52.083333, swego.SunshineAlt, nil},
		housesTestResult{
			[]float64{0,
